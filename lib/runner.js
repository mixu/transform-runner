...
Runner.prototype.hasCached = function(filename) {
  ...
  rawDeps = this.cache.get(filename, 'rawDeps');
  // - a set of normalized deps
  resolvedDeps = this.cache.get(filename, 'resolvedDeps');
...
Runner.prototype.parseAndUpdateDeps = function(filename, cacheFile, done) {
  ...
  detectiveDependencies(filename, cacheFile, this.opts.ignore, this.dependencyCache,
    this.resolverOpts, function(err, rawDeps, resolvedDeps, renames) {
    // do not store result when an error occurs
    if (!err) {
      // self.log.info('Cache parse result:', filename);
      // store the dependencies
      self.cache.set(filename, 'rawDeps', rawDeps);
      // store the normalized dependencies
      self.cache.set(filename, 'resolvedDeps', resolvedDeps);
