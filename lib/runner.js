...
function Runner(opts) {
  ...
  if (opts.exclude) {
    // assert all all regexps
    if(opts.exclude.some(function(re) { return !(re instanceof RegExp); })) {
      throw new Error('opts.exclude must only contain RegExps!' +
        JSON.stringify(opts.exclude.map(function(re) { return re.toString(); })));
    }
    this.excludeChecks.push(function(filename) {
      return !opts.exclude.some(function(expr) {
        if (filename.match(expr)) {
          self.log.info('Excluded by regexp', expr.toString(), ':', filename);
          return true;
        }
      });
    });
...
Runner.prototype.add = function(filename) {
  ...
  self._queue.exec(function(done) {
    self.hasCached(filename, function(err, result) {
      ...
      // Apply exclusions
      var isExcluded = self.excludeChecks.some(function(fn) {
        return !fn(filename);
      });
      if (isExcluded) {
        self.log.info('File excluded', filename);
        return done();
      }
      ...
      // Apply --ignore's
      if (self.ignore) {
        var isIgnored = self.ignoreChecks.some(function(fn) {
          return !fn(filename);
        });
        if (isIgnored){
          self.log.info('File ignored', filename);
